...
exports = module.exports = function (/*optional*/xs, /*optional*/ee, /*optional*/trace) {
    return new Stream(xs, ee, trace);
};
...
function Stream(/*optional*/xs, /*optional*/ee, /*optional*/trace) {
    ...
    self._trace = !!trace
        ? (isFunction(trace) ? trace : _.log.bind(_.log, self.id))
        : noop;
    this.paused = true;
    this._incoming = [];
    this._outgoing = [];
...
Stream.prototype.consume = function (f) {
    ...
    var push = function (err, x) {
        self._trace('push', err, x, s.paused);
        if (x === nil) {
...
Stream.prototype.fork = function () {
    ...
    s.id = 'fork:' + s.id;
    s.source = this;
    s._trace = this._trace.bind(this, s.id);
    this._consumers.push(s);
    this._checkBackPressure();
    return s;
};
 ...
 * };
 *
 * _(docs).group(f); || _(docs).group('blogpost');
 * // => {
