 ...
 * @id slidingBuffer
 * @section Streams
 * @name Stream.slidingBuffer(n)
 * @param {Number} n - number of elements in window
 * @param {Number} m > 0 - number of elements to skip between windows (default 1)
 * @api public
 *
 * _([1, 2, 3, 4, 5]).slidingBuffer(3, 1)  // => [1, 2, 3], [2, 3, 4], [3, 4, 5]
...
Stream.prototype.slidingBuffer = function (n, m) {
    ...
    return this.consume(function (err, x, push, next) {
        if (err) {
          ...
          if (!windowFilled && windowed.length > 0) {
              push(null, windowed);
          }
          push(null, nil);
        }
            ...
            if (n < 1) {
              windowed.push(x);
                ...
                windowFilled = false;
                windowed.push(x);
                if (windowed.length === n) {
                    windowFilled = true;
                    push(null, windowed);
                    windowed = windowed.slice(m);
                }
            }
            next();
        }
