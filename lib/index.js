...
/**
 * A moving window over the stream which returns arrays of length <= count while skipping skip elements.
 *
 * @id slidingBuffer
 * @section Streams
 * @name Stream.slidingBuffer(count, skip)
 * @param {Number} count - number of elements in window
 * @param {Number} skip > 0 - number of elements to skip between windows
 * @api public
 *
...
Stream.prototype.slidingBuffer = function (count, skip) {
    var windowed = [];
    var missing = 0;
    ...
    if (typeof skip != 'number' || skip <= 0) {
        throw new Error('buffer skip must be >0');
    }
    ...
    return this.consume(function (err, x, push, next) {
        if (err) {
            ...
            while (windowed.length) {
                push(null, windowed);
                windowed = windowed.slice(skip);
            }
            push(null, nil);
        }
        else {
            if (missing > 0) {
                ...
                windowed.push(x);
                if (windowed.length === count) {
                    push(null, windowed);
                    missing = Math.max(skip - count, 0);
                    windowed = windowed.slice(skip);
                }
            }
            next();
