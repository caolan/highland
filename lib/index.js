...
Stream.prototype.parallel = function (n, k) {
    var source = this;
    return this
      .buffer(n-1)
      .map(x => x.buffer(k || Number.MAX_VALUE))
      .sequence()
};
exposeMethod('parallel');

Stream.prototype.buffer = function (n, k) {
  var xs = []
  var reading = false
  var waiting = false
  var running = true

  var pull = function(next) {
    if (reading || !running) {
      return
    }
    if (xs.length != 0 && xs.length >= 0) {
      return
    }
    reading = true
