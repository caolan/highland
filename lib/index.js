...
/**
 * A moving window over the stream which returns arrays of length <= n while skipping m elements.
 *
 * @id slidingBuffer
 * @section Streams
 * @name Stream.slidingBuffer(n,m)
 * @param {Number} n - number of elements in window
 * @param {Number} m > 0 - number of elements to skip between windows
 * @api public
 *
 * _([1, 2, 3, 4, 5]).slidingBuffer(3, 1)  // => [1, 2, 3], [2, 3, 4], [3, 4, 5], [4, 5], [4]
 * _([1, 2, 3, 4, 5]).slidingBuffer(2, 2)  // => [1, 2], [3, 4], [5]
 */

Stream.prototype.slidingBuffer = function (n, m) {
    //number of values to skip after a complete window
    //relevant only when m > n
    var toskip = m - n;
    var state = {
      windowed: [],
      skipped: toskip
    ...
    function transform(state){
      var w = state.windowed;
      var s = state.skipped;
      var x = state.x;
      var emit = null;
      //stream has ended and we have no more values in buffer so end
      if (x === nil && w.length === 0){
        return null;
      }
      //stream has ended and we have values in buffer to process
      if (x === nil && s >= toskip){
        emit = w;
        w = w.slice(m);
        s = 0;
      }
      //our buffer is full so emit it and transform
      if (w.length === n){
        emit = w;
        w = w.slice(m);
        s = 0;
      }
      //if we have skipped enough, add to buffer
      if (s >= toskip && w.length < n){
        if (x !== nil) {
          w.push(x);
        }
      }
      ...
      s++;
      return {emit: emit, windowed: w, skipped: s};
    }
    ...
    return this.consume(function (err, x, push, next) {
        ...
        if (err) {
          ...
          while ((state = transform(state))){
            if (state.emit !== null){
              push(null, state.emit);
            }
            state.x = x;
          }
          push(null, nil);
