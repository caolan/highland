...
Stream.prototype._redirect = function (to) {
    ...
    // for now, just throw an error instead
...
function consumePush(err, x) {
    console.log(['consumePush', this.id, err, x]);
    if (x === nil) {
        this.source._removeConsumer(this);
    }
    if (this.paused) {
        if (err) {
            this._outgoing.push(new StreamError(err));
        }
        else {
            this._outgoing.push(x);
        }
    }
    else {
        console.log(['sending', err, x]);
        this._original_send(err, x);
    }
}

function consumeNext(s2) {
    console.log(['consumeNext', this.id, s2 && s2.id]);
    if (s2) {
        // we MUST pause to get the redirect object into the _incoming
        // buffer otherwise it would be passed directly to _send(),
        // which does not handle StreamRedirect objects!
        var _paused = this.paused;
        if (!_paused) {
            this.pause();
        }
        this.write(new StreamRedirect(s2));
        if (!_paused) {
            this.resume();
        }
    }
    else if (this._consume_async) {
        console.log(['this._consume_async === true', this.id]);
        this.resume();
    }
    else {
        console.log(['this._consume_async === false', this.id]);
        this._consume_next_called = true;
    }
};

function consumeSend(err, x) {
    console.log(['consumeSend', this.id, err, x]);
    this._consume_async = false;
    this._consume_next_called = false;
    this._consume_f(
        err,
        x,
        this._consume_push.bind(this),
        this._consume_next.bind(this)
    );
    this._consume_async = true;
    if (!this._consume_next_called) {
        console.log(['this._consume_next_called === false, pausing', this.id]);
        this.pause();
    }
};

Stream.prototype.consume = function (f) {
    console.log(['consume', this.id, this._redirected]);
    if (this._redirected) {
        throw new Error(
            'This stream has been redirected using next() and you can no ' +
            'longer attach consumers to it'
        );
    }
    var s = new Stream();
    s._consume_push = consumePush;
    s._consume_async = false;
    s._consume_next_called = false;
    s._consume_next = consumeNext;
    s._original_send = s._send;
    s._send = consumeSend;
    s._consume_f = f;
    this._addConsumer(s);
    return s;
