...
/**
 * Reduces a stream, using a stream-returning function. This allows async
 * reduction. The function is called once for each stream value in the
 * previous step, and the resulting streams are concatenated together,
 * similar to [flatMap](#flatMap).
 *
 * If the iterator throws an error, the reduction stops and the resulting
 * stream will emit that error instead of a value.
 *
 * @id flatReduce
 * @section Transforms
 * @name Stream.flatReduce(memo, iterator)
 * @param memo - the initial state of the reduction
 * @param {Function} iterator - the function which reduces the values
 * @api public
 *
 ...
 * };
 *
 * _([1, 2, 3, 4]).flatReduce(0, add)  // => 10
 */

Stream.prototype.flatReduce = function(z, f) {
    var z_ = _([z]);
    return this.consume(function(err, x, push, next) {
        if (x === nil) {
            next(z_);
...
exposeMethod('flatReduce');

/**
 * Same as [flatReduce](#flatreduce), but uses the first element as the initial
 * state instead of passing in a `memo` value.
 *
 * @id flatReduce1
 * @section Transforms
 * @name Stream.flatReduce1(iterator)
 * @param memo - the initial state of the reduction
 * @param {Function} iterator - the function which reduces the values
 * @api public
 *
 ...
 * };
 *
 * _([1, 2, 3, 4]).flatReduce1(add)  // => 10
 */

Stream.prototype.flatReduce1 = function(f) {
    var self = this;
    return _(function (push, next) {
        self.pull(function (err, x) {
            if (err) {
                ...
                next(self.flatReduce(x, f));
            }
        });
    });
};
exposeMethod('flatReduce1');
