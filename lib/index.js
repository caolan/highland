...
/**
 * Collects all values from a Stream into an Array and calls a node style
 * callback with the result, or a error. This function causes a **thunk**.
 *
 *
 * @id nodestyleToArray
 * @section Consumption
 * @name Stream.nodestyleToArray(f)
 * @param {Function} cb - the node-style callback
 * @api public
 *
 * _([1, 2, 3, 4]).nodestyleToArray(function (err, x) {
 *     // parameter x will be [1,2,3,4]
 * });
 */

Stream.prototype.nodestyleToArray = function (cb) {
    return this.collect().pull(cb);
};
