 ...
 * @api public
 *
 * var asyncUnit = () => h.of(h.nil);
 * var asyncAutocomplete = e => h(fetch(`/autocomplete?s=${e.target.value}`));
 *
 * var fns = h('change', checkbox)
 *   .flatMap(e => h.of(e.target.checked ? asyncAutocomplete : asyncUnit));
 *
 * h('change', input)
 *   .ap(fns)
 *   .merge()
