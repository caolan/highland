...
function Stream(generator) {///*optional*/xs, /*optional*/ee, /*optional*/mappingHint) {
    ...
    self._push_fn = this._push.bind(this);
    self._next_fn = this._next.bind(this);
...
Stream.prototype._push = function _push(err, x) {
    // console.log('_push', err, x);
    this._writeOutgoing(err ? new StreamError(err) : x);
};

Stream.prototype._next = function _next(xs) {
    // console.log(this.id, '_next', xs, this.paused);
    this._generator_running = false;
    if (xs) {
        var xs = _(xs);
        var pull = xs.pull.bind(xs);
        this._generator = newDelegateGenerator(pull);
    }
...
Stream.prototype.pull = function (f) {
    ...
    // console.log(self.id, 'pull', self._outgoing.toArray(), self.paused);
    if (self._request) {
    ...
    if (self._outgoing.length) {
        ...
        self._request = function request(err, x) {
            self.pause();
            f(err, x);
        }
        self.resume();
...
function ConsumeStream(source, f) {
    ...
    ConsumeStream.super_.call(self, function (push, next) {
        source.pull(function (err, x) {
            // console.log(self.id, 'consume', err, x, a.paused);
            f(err, x, push, next);
        });
    });
...
StreamMultiplexer.prototype.paused = function paused() {
    return this._requests !== this._numConsumers;
};

StreamMultiplexer.prototype.pull = function pull(key, cb) {
    if (!cb) {
        return;
    }

    if (this._consumers[key]) {
        cb(new Error('Cannot service a second pull() request while one is in progress.'));
        return;
    }

    this._consumers[key] = cb;
    this._requests++;
