 ...
 * @id mergeLimit
 * @section Higher-order Streams
 * @name Stream.mergeLimit()
 * @api public
 *
 * var txt = _(['foo.txt', 'bar.txt']).map(readFile)
 * var md = _(['baz.md']).map(readFile)
 * var js = _(['bosh.js']).map(readFile)
 *
...
Stream.prototype.mergeLimit = function (n){
    var count = 0;
    var done = false;
    if (!(n > 0)) {
        throw new Error('limit must be a number above 0');
    }
    return this.consume(function(err, stream, push, next){
        if (err) {
            push(err);
        }
        else if (stream === _.nil) {
            // console.log('end of Stream of Streams', count);
            if (count === 0) {
                // console.log('push nil');
                push(null, _.nil);
            }
            done = true;
            return;
        }
        else if (_.isStream(stream)) {
            count = count + 1;
            // console.log('start', stream.id,  count);
            stream.map(push.bind(push, null))
            .toArray(function(){
                count = count - 1;
                // console.log('finished', stream.id, count, done);
                if (done) {
                    if (count === 0) {
                        // console.log('push nil');
                        push(null, _.nil);
            ...
            throw new Error('Expected stream, got ' + typeof stream);
        }
        if (count < n) {
            next();
        }
