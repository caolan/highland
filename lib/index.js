 ...
 * @id toPromise
 * @section Consumption
 * @name Stream.toPromise()
 * @api public
 *
 * _([1, 2, 3, 4]).toPromise().then(function (x) {
 *     // parameter x will be [1,2,3,4]
 * });
 */
...
/**
 * Creates a new Stream of streams by applying each item to a function returning
 * either a promise, generator or another stream.
 *
 * @id asyncMap
 * @section Higher-order Streams
 * @name Stream.asyncMap(f)
 * @param {Function} f - the async function
 * @api public
 *
 * filenames.asyncMap(async (x) => await upload(x))
 */
