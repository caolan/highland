 ...
 * from its source as fast as the slowest consumer can handle them. For an
 * example of the difference between `fork` and `observe`, see [this
 * post](https://github.com/caolan/highland/issues/561#issuecomment-257069894).
 *
 ...
 *
 * const xs = _([1, 2, 3, 4]);
 * const ys = xs.fork().flatMap((x) => delay(x, 1000));
 * const zs = xs.fork().flatMap((x) => delay(x, 2000));
 *
 * // No values will be pulled from xs until zs also starts consuming.
 * ys.each((x) => console.log(`ys: ${x}`));
 *
 * // Now both ys and zs will get values from xs as fast as they both
 * // can process them.
 * zs.each((x) => console.log(`zs: ${x}`));
 *
 * // =>
 * // ys: 1
 * // zs: 1
 ...
 * them. For an example of the difference between `fork` and `observe`, see
 * [this
 * post](https://github.com/caolan/highland/issues/561#issuecomment-257069894).
 *
 ...
 *
 * const xs = _([1, 2, 3, 4]);
 * const ys = xs.observe().flatMap((x) => delay(x, 1000));
 * const zs = xs.flatMap((x) => delay(x, 10));
 *
 * // ys will not receive any data yet, since it is only a passive observer.
 * ys.each((x) => console.log(`ys: ${x}`));
 *
