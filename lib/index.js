...
/**
 * Creates a new Stream yielding values from the source while the given
 * function returns true for the passed value.
 *
 * @id takeWhile
 * @section Transforms
 * @name Stream.takeWhile(f)
 * @param {Function} f - callback returning false to stop the stream
 * @api public
 *
 * _([1, 2, 3, 4]).takeWhile(function (x) {
 *   return x < 3;
 * }) // => 1, 2
 */
...
exposeMethod('takeWhile');

/**
 * Creates a new Stream yielding values from the source until the given
 * function returns true for the passed value.
 *
 * @id takeUntil
 * @section Transforms
 * @name Stream.takeUntil(f)
 * @param {Function} f - callback returning true to stop the stream
 * @api public
 *
 * _([1, 2, 3, 4]).takeUntil(function (x) {
 *   return x >= 3;
 * }) // => 1, 2
 */

Stream.prototype.takeUntil = function (f) {
    return this.takeWhile(function (x) {
        return !f(x);
    });
};
exposeMethod('takeUntil');
