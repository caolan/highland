...
/**
 * Ends a source stream when a given stream ends or emits a value.
 *
 * @id takeUntil
 * @section Higher-order Streams
 * @name Stream.takeUntil(stream)
 * @param {Stream} stream - another stream which stops the source
 * @api public
 *
 * a = _()
 * b = _()
...
Stream.prototype.takeUntil = function (stream) {
    if (!_.isStream(stream)) {
        throw new Error('Invalid stream to takeUntil:', stream);
    }
    var first = true,
        done = false;
