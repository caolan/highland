...
/**
 * Writes values from Highland Stream 'src' to Node.js Writable Stream 'dest'.
 * When the destination returns false from a write() call, it will attempt to
 * pause the source to avoid buffering.
 *
 * Be aware that Node.js writable streams only work with Strings or Buffers.
 *
 * @name pipe -> Stream -> Node.js Writable Stream
 * @param {Stream} a - the source stream
 * @param {Stream} b - the destination writable stream
 * @api public
 *
...
h.pipe = function (src, dest) {
    var await_drain = false;
    dest.on('drain', function () {
        if (await_drain) {
            src._requestResume(dest);
        }
        await_drain = false;
    });
    src.on('data', function (x) {
        var written = dest.write(x);
        if (written === false) {
            src._requestPause(dest);
            await_drain = true;
        }
    });
    src.on('end', function () {
...
h.cons = h.curry(function (val, xs) {
    if (h.isStream(xs)) {
        var ys = h.createStream(xs);
        ys.push(val);
...
h.append = h.curry(function (val, xs) {
    if (h.isStream(xs)) {
        var ys = h.createStream(xs);
        xs.on('data', function (x) {
            ys.push(x);
...
/**
 * Takes a value and an Array or Stream and places the value between each
 * adjacent element in the Array or Stream. Similar to calling join on an Array
 * only the values are not joined into a single String, instead the separator is
 * added as an element to the Array.
 *
 * @name intersperse -> v -> xs -> Array | Stream
 * @param v - the value to intersperse between elements
 * @param xs - the array or stream to intersperse the value in
 * @api public
 *
 * intersperse(0, [1,2,3]) == [1,0,2,0,3]
 */

h.intersperse = h.curry(function (v, xs) {
    if (h.isStream(xs)) {
        var first = true;
        var ys = h.createStream(xs);
        xs.on('data', function (x) {
...
/**
 * Returns a filtered array where all properties of the matched objects are
 * equal to the provided attrs by a shallow comparison using eq().
 *
 * @name where attrs -> xs -> Array
 * @param {Object} attrs - the attributes to match
 * @param {Array} xs - the array to filter
 * @api public
 *
 * var things = [
 *     {owner: 'sally', item: 'umbrella'}
 *     {owner: 'david', item: 'coat'},
 *     {owner: 'sally', item: 'book'},
 * ];
 * where({owner: 'sally'}, things) == [
 *     {owner: 'sally', item: 'book'},
 *     {owner: 'sally', item: 'coat'},
 * ]
 */

h.where = h.curry(function (attrs, xs) {
...
/**
 * Similar to where(), but like find() will only return the first matching
 * result.
 *
 * @name findWhere attrs -> xs -> Array
 * @param {Object} attrs - the attributes to match
 * @param {Array} xs - the array to find items from
 * @api public
 */

h.findWhere = h.curry(function (attrs, xs) {
    return h.find(function (x) {
        for (var k in attrs) {
            if (!h.eq(attrs[k], x[k])) {
                return false;
            }
...
/**
 * Tests if an item exists in an Array, returning true or false.
 *
 * @name contains -> y -> xs -> Array
 * @param y - the value to check for
 * @param {Array} xs - the array to search through
 * @api public
 *
 * contains('foo', ['foo', 'bar']) == true
 * contains('wibble', ['foo', 'bar']) == false
 */

h.contains = h.curry(function (y, xs) {
    for (var i = 0, len = xs.length; i < len; i++) {
        var x = xs[i];
        if (h.eq(x, y)) {
            return true;
 ...
 *
 * @name without -> y -> xs -> Array
 * @param y - the value to remove
 * @param xs - the array to remove values from
 * @api public
 */

h.without = h.curry(function (y, xs) {
    return h.filter(function (x) {
        return x !== y;
    }, xs);
});
...
/**
 * Browser port of util.inherits from Node.js, taken from node-browser-builtins
 * https://github.com/alexgorbatchev/node-browser-builtins/blob/master/builtin/util.js
 */
...
var Stream = h.Stream = function Stream(parent) {
    ...
    var that = this;
    var onNewListener = function (name, handler) {
        if (name === 'data' && that.buffer.length) {
            while (that.buffer.length) {
                handler(that.buffer.shift());
            }
        }
        if (name === 'end' && that.ended) {
            handler();
        }
    };
    this.on('newListener', onNewListener);
};
inherits(h.Stream, EventEmitter);

Stream.prototype._requestPause = function (from) {
    this.holds.push(from);
    if (this.pause && !this.paused) {
        this.pause();
        this.paused = true;
    }
    if (this.parent) {
        this.parent._requestPause(this);
    }
};

Stream.prototype._requestResume = function (from) {
    this.holds = this.holds.filter(function (h) {
        return h !== from;
    });
    if (!this.holds.length) {
        if (this.resume && this.paused) {
            this.resume();
            this.paused = false;
        }
        if (this.parent) {
            this.parent._requestResume(this);
        }
    }
};
...
/**
 * Add the following Highland functions as methods on Stream objects
 */
...
// the argument order is different for this function, so adding manually
Stream.prototype.pipe = function (dest) {
    return h.pipe(this, dest);
};



h.createStream = function (parent) {
