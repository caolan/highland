...
exports['tracing - using built in log'] = function (test) {
//  test.expect(2);
    function doubled(x) {
        return x * 2;
    }
    ...
    test.equals(calls, 13);

    calls = 0;

    var s = _([1, 2, 3, 4], null, true);
    var xs = s.fork(), ys = s.fork();

    ys.map(doubled)
        .reduce1(function (m, x) { return m + x; })
        .toArray(function (xs) {
            test.same(xs, [20]);
        });
...
exports['tracing - using supplied fn'] = function (test) {
